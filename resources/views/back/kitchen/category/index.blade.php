                ...
                <div class="card card-fluid" style="margin-top:1rem;">
                        ...
                        <div class="card-body">
                            <div id="admin_gallery">
                                ...
                                @include('back.kitchen.category.add')
                                @include('back.kitchen.category.edit')
                                <div class="  ">
                                   
                                    <div class="row" id="categoryHolder">
                                       
                                            @foreach ($cats  as $cat)
                                              @include('back.kitchen.category.single',['cat'=>$cat])
                                            @endforeach
...
<script>
    ...
    function addCategory(){
        ...
        if(!addLock){
            ...
            axios.post('{{route("admin.kitchen.category.add")}}',fd, {
                headers: {
    ...
    function updateCategory(){
        ...
        if(!addLock){
            ...
            axios.post('{{route("admin.kitchen.category.update")}}',fd, {
                headers: {
                'Content-Type': 'multipart/form-data'
                }
            })
    ...
    function delCategory(id){
        axios.post('{{route("admin.kitchen.category.del")}}',{"id":id})
            .then((response)=>{
                $('#cat_'+id).remove();
            });
    }
